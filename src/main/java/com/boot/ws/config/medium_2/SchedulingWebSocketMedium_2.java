...
@EnableScheduling
@Component
public class SchedulingWebSocketMedium_2 implements SchedulingConfigurer {

	private static Logger logger = LogManager.getLogger(SchedulingWebSocketMedium_2.class);
	...
	public void configureTasks(ScheduledTaskRegistrar taskRegistrar) {
		...
		Runnable runnable = () -> {
			...
			ResponseEntity<Response> response = restTemplate.exchange(Constant.API_URL + "name=medium-2",
					HttpMethod.GET, requestEntity, Response.class);
			if (response.getBody() != null) {
				Response res = response.getBody();
				logger.info("API Response for medium-2 =====> " + res + ", status : " + res.getStatus()
				...
				simpMessagingTemplate.convertAndSend("/topic/sibsankar-system-medium-2", response);
			}
		};
